        ...
        <table id="example" class="table table-striped table-bordered display" style="width:100%">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $row->email }}</td>
                    <td>
                        <a href="{{ route('pemilik.show', $row->id) }}" title="Lihat Detail">
                            <span class="label label-info"><i class="fa fa-eye"></i> Lihat</span>
                        </a>
                        <a href="{{ route('pemilik.edit', $row->id) }}" title="Ubah Data">
                            <span class="label label-primary"><i class="fa fa-edit"></i> Ubah</span>
                        </a>
                        <form method="POST" action="{{ route('pemilik.destroy', $row->id) }}" style="display: inline-block;">
                            @method('DELETE')
                            @csrf
